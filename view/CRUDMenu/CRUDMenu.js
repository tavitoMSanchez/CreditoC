...
function editar(IdMenu){
    $.post("../../controller/menu.php?op=mostrar",{IdMenu : IdMenu}, function(data, status){
        data = JSON.parse(data);
        $('#titulo_crud').html('CRUD-Editar Registro');
        $('#men_ruta').val(data.Menu);
        $('#men_icon').val(data.MenuIcon);
        $('#men_nom').val(data.MenNom);
        $('#men_id').val(data.IdMenu);
    }); 
    $("#modalcrud").modal('show');	
}
...
function eliminar(IdMenu){
    swal.fire({
        ...
        if (result.value) {
            $.post("../../controller/menu.php?op=activarydesactivar",{IdMenu : IdMenu}, function(data, status){
